...
export const ru: Strings = {
  ...
  // ------------------------------------------------------------ 失敗の知らせ方
  failNetwork: "Нет подключения к сети. Синхронизация продолжится сама, как только связь вернётся. Чтобы проверить сейчас, нажмите «Проверить состояние».",
  failServer: "Google Drive сейчас не отвечает. Синхронизация продолжится сама через некоторое время.",
  failAuth: "Срок входа в Google истёк. Подключитесь заново в настройках.",
  failTarget: "Не удаётся открыть папку синхронизации. Проверьте, не в корзине ли она и остался ли доступ к общему диску.",
  failQuota: "На Drive закончилось место или достигнут предел числа файлов. Обратитесь к администратору.",
  failDetail: (m) => `подробности: ${m}`,
  btnOpenSettings: "Открыть настройки",
  tipOpenSettings: "Открыть настройки плагина для повторного подключения",
  failRetryIn: (x) => `повтор примерно через ${x}`,
