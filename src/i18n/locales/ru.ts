...
export const ru: Strings = {
  ...
  // ------------------------------------------------ ローカル固有ファイルの分類
  sectionUnsorted: "Не разобрано",
  sectionShared: "Поделиться",
  sectionTrash: "Удалить",
  localOnlyIntro:
    "Файлы, которые есть в этом хранилище и нет на Диске. Перенесите каждую строку в «Поделиться» или «Удалить», затем выполните «Разобрать локальные файлы». Этот файл никогда не синхронизируется.",
  ...
  btnOrganize: "Разобрать локальные файлы",
  panelUnsorted: "Не разобрано",
  panelHeldUploads: "Приостановленные отправки",
  panelLocalOnlyDesc: "Отправка новых локальных файлов приостановлена, пока список «Не разобрано» не опустеет.",
  btnOpenLocalOnly: "Открыть список",
  organizeDone: (shared, trashed) => `локальные файлы разобраны — к отправке ${shared}, в корзину ${trashed}`,
